  ...
  const fetchBillings = async () => {
    try {
      const response = await api.get(
        `/receptionist/allBilling?page=${currentPage}&limit=${limit}`
      );
        ...
        <div className="TotalBillingRecepGrid">
          {billings.map((bill) => (
            <div key={bill._id} className="TotalBillingRecepCard">
              <h3 className="TotalBillingRecepPatient">
                👤 {bill.patient.name}
              </h3>
              ...
              <p className="TotalBillingRecepTotal">
                💰 Total: ₹{bill.totalAmount}
              </p>
              <p>🕒 {new Date(bill.createdAt).toLocaleString()}</p>
              <p className={`TotalBillingRecepStatus ${bill.status}`}>
                {bill.status}
              </p>
            </div>
          ))}

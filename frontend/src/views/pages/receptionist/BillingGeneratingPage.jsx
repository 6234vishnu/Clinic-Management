...
const BillingGeneratingPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.additionalCharges && !formData.consultationFee)
      return setmessage("fill the feilds before the submission");
    try {
      const response = await api.post(
        `/receptionist/generateBilling/${PrescriptionId}`,
        { formData }
      );
      ...
      <div className="BillingGenarationRecepContainer">
        <form className="BillingGenarationRecepForm" onSubmit={handleSubmit}>
          ...
          <h6 style={{ color: "green" }}>
            <strong>Note: </strong>If this is the first appoinment token fee{" "}
            <strong>₹100</strong> will be added
          </h6>
      ...
      {uploadedBill && uploadedBill._id && (
        <div className="BillingGenarationRecepModalOverlay">
          <div className="BillingGenarationRecepModal">
            <h2 className="BillingGenarationRecepModalTitle">
              Billing Details
            </h2>
            <p>
              <strong>Patient Name</strong> {uploadedBill.patient.name}
            </p>
            <p>
              <strong>Consultation Fee:</strong> ₹{uploadedBill.consultationFee}
            </p>
            <p>
              <strong>Additional Charges:</strong> ₹
              {uploadedBill.additionalCharges}
            </p>
            <p>
              <strong>Total Amount:</strong> ₹{uploadedBill.totalAmount}
            </p>
            <p>
              <strong>Date:</strong>{" "}
              {new Date(uploadedBill.createdAt).toLocaleString()}
            </p>
            <h6 style={{ color: "green" }}>
              <strong>Note: </strong>If this is the first appoinment token fee{" "}
              <strong>₹100</strong> will be added
